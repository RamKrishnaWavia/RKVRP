...
def validate_columns(df):
    ...
    for col in ['Latitude', 'Longitude', 'Orders', 'Hub ID']:
        df[col] = pd. produce this simplified format. The new logic will:
*   Show only the society names if there is one or more delivery points.
*   Exclude the `Depot ->` at the beginning and the `-> Depot (...)` at the end.
*   For a cluster with a single society, it will now just show that society's name.

---

### Complete Corrected Code

Here is the final version of `app.py`. The only change is inside the `create_summary_df` function to alter how the `Delivery Sequence` string is built.
...
def validate_columns(df):
    ...
    if df.isnull().values.any():
        return "File contains non-numeric or empty values in required numericto_numeric(df[col], errors='coerce')
    if df.isnull().values.any():
...
def get_distance_to_last_society(points, depot_coord, circuity_factor):
 ...
 columns (Lat, Lon, Orders, Hub ID). Please check."
    return None
...
def create_summary_df(clusters, depot_coord, circuity_factor):
    """
    Creates the summary DataFrame with a simplified, societies-only delivery sequence.
    """
    summary_rows = []
    for c in clusters:
        total_orders, cpo = c['Orders'], (c['Cost'] / c['Orders                    path, distance = get_delivery_sequence(potential_cluster, depot_coord, circuity_factor)
                    all_clusters.append({'Cluster ID': f"{cluster_type}-{cluster_id_counter}", 'Type': cluster_type, 'Societies': potential_cluster, 'Orders': potential_orders, 'Distance': distance, 'Path': path, 'Cost': costs[cluster_type.lower()]})
                    cluster_id_counter += 1; hub_society_ids -= {s['Society ID'] for s in potential_cluster}
        for sid in hub_society_ids:
            society = societies_map[sid]
...
def create_summary_df(clusters, depot_coord, circuity_factor):
    ...
    for c in clusters:
        ...
        # --- NEW: Build the delivery sequence string without Depot ---
        delivery_sequence_str = ""
        if not c['Path']:
            delivery_sequence_str = "N/A" # Should not happen, but a safeguard
        elif len(c['Path']) == 1:
            # For a single point, just show the name
            delivery_sequence_str = id_to_name.get(c['Path'][0])
        else:
            # For multiple points, build the A -> B (dist) -> C (dist) chain
            path_ids = c['Path']
            sequence_parts = [id_to_name.get(path_ids[0])] # Start with the first society name
            for i in range(1, len(path_ids)):
                prev_coord = id_to_coord[path_ids[i-1]]
                curr_coord = id_to_coord[path_ids[i]]
                dist = calculate_route_distance(prev_coord, curr_coord, circuity_factor)']) if c['Orders'] > 0 else 0
        id_to_name, id_to_coord = {s['Society ID']: s['Society Name'] for s in c['Societies']}, {s['Society ID']: (s['Latitude'], s['Longitude']) for s in c['Societies']}
        ...
        # --- NEW: Build the simplified, societies-only delivery sequence string ---
        delivery_sequence_str = ""
        if c['Path']:
            if len(c['Path']) == 1:
                ...
                for i in range(1, len(c['Path'])):
                    ...
                    dist = calculate_route_distance(start_coord,
                sequence_parts.append(f"-> {id_to_name.get(path_ids[i])} ({dist:.2f} km)")
    ...
    return pd. end_coord, circuity_factor)
...
def create_unified_map(clusters, depot_coord, circuity_factor):
    ...
    colors = {'Main': 'blue', 'Mini': 'green', 'Micro': 'purpleDataFrame(summary_rows)
...
def create_unified_map(clusters, depot_coord, circuity_factor):
    ...
    for c in clusters:
        ...
        id_to_name, id_to_coord = {s['Society ID']: s['Society Name'] for s in c['Societies']},', 'Unclustered': 'red'}
    for c in clusters:
        color, fg = colors.get(c['Type'], 'gray'), folium.FeatureGroup(name=f"{c['Cluster ID']} ({c['Type']})")
        id_to_name, id_to_coord = {s['Society ID']: s['Society Name'] for s in c['Societies']}, {s['Society ID']: (s['Latitude'], s['Longitude']) for s in c['Societies']}
        full_path_info = [("Depot", depot_coord)] + ([(id_to_name.get(sid, str(sid {s['Society ID']: (s['Latitude'], s['Longitude']) for s in c['Societies']}
        full_path_info = [("Depot", depot_coord)] + ([(id_to_name.get(sid, str(sid)), id_to_coord.get(sid)) for sid in c[')), id_to_coord.get(sid)) for sid in c['Path']] if c['Path'] else []) + [("Depot", depot_coord)]
        for i in range(len(full_path_Path']] if c['Path'] else []) + [("Depot", depot_coord)]
        for i ininfo) - 1):
            start_name, start_coord = full_path_info[i]; range(len(full_path_info) - 1):
            start_name, start_coord = end_name, end_coord = full_path_info[i+1]
            if start_coord full_path_info[i]; end_name, end_coord = full_path_info[i+ and end_coord:
                dist = calculate_route_distance(start_coord, end_coord, circ1]
            if start_coord and end_coord:
                dist = calculate_route_distance(startuity_factor)
                folium.PolyLine(locations=[start_coord, end_coord], color=color, weight_coord, end_coord, circuity_factor)
                folium.PolyLine(locations=[start_coord, end_coord], color=color, weight=2.5, opacity=0.8, tooltip=f"{start_name}=2.5, opacity=0.8, tooltip=f"{start_name} to {end_name}: {dist:.2f} km").add_to(fg)
        for society in c['Societies']: folium to {end_name}: {dist:.2f} km").add_to(fg)
        for society.Marker(location=[society['Latitude'], society['Longitude']], popup=f"<b>{society['Society Name']}</b><br>Orders: {society['Orders']}<br>Cluster: {c['Cluster ID']}", icon in c['Societies']: folium.Marker(location=[society['Latitude'], society['Longitude']], popup=f"<b>{society['Society Name']}</b><br>Orders: {society['Orders']}<br>Cluster:=folium.Icon(color=color, icon='info-sign')).add_to(fg)
         {c['Cluster ID']}", icon=folium.Icon(color=color, icon='info-sign')).fg.add_to(m)
    folium.LayerControl().add_to(m)
    return m

# --- STREAMLIT UI ---
st.markdown("<div style='text-align: center;'>add_to(fg)
        fg.add_to(m)
    folium.LayerControl().add_to(m)
    return m

# --- STREAMLIT UI ---
st.markdown("<div style='text<h1> 🚚 RK - Delivery Cluster Optimizer and Sequencing</h1></div>", unsafe_allow_html=True)

with st.sidebar:
    st.header("1. Depot Settings")
    depot_lat = st.number-align: center;'><h1> 🚚 RK - Delivery Cluster Optimizer and Sequencing</h1></div>", unsafe_allow_html=True)

with st.sidebar:
    st.header("1. Depot Settings")
    depot__input("Depot Latitude", value=12.9716, format="%.6f")
lat = st.number_input("Depot Latitude", value=12.9716, format="%.6f")
    depot_long = st.number_input("Depot Longitude", value=    depot_long = st.number_input("Depot Longitude", value=77.5946, format="%.6f")
    depot_coord = (depot_lat, depot_77.5946, format="%.6f")
    depot_coord = (depotlong)

    st.header("2. Routing & Cost Settings")
    circuity_factor = st._lat, depot_long)

    st.header("2. Routing & Cost Settings")
    circuity_factor = st.slider("Circuity Factor (for driving distance estimation)", 1.0, 2.slider("Circuity Factor (for driving distance estimation)", 1.0, 2.0, 1.0, 1.4, 0.1, help="Adjust to estimate driving distance from straight-line distance4, 0.1, help="Adjust to estimate driving distance from straight-line distance. 1.4 = 40% longer.")
    
    costs = {'main': st.number_input("Main Cluster Van Cost (. 1.4 = 40% longer.")
    
    costs = {'main': st.number_input("Main Cluster Van Cost (₹)", 833) + st.number_input("Main Cluster₹)", 833) + st.number_input("Main Cluster CEE Cost (₹)", 333),
             'mini': st.number_input("Mini Cluster Van Cost (₹)", 10 CEE Cost (₹)", 333),
             'mini': st.number_input("Mini Cluster Van Cost (₹)", 1000) + st.number_input("Mini Cluster CEE Cost (00) + st.number_input("Mini Cluster CEE Cost (₹)", 200),
             'micro': st.number_input("Micro Cluster Van Cost (₹)", 500) + st₹)", 200),
             'micro': st.number_input("Micro Cluster Van Cost (₹.number_input("Micro Cluster CEE Cost (₹)", 200)}

    st.header("3. Upload Data")
    file = st.file_uploader("Upload Society Data CSV", type=["csv"]))", 500) + st.number_input("Micro Cluster CEE Cost (₹)", 200)}
...
if st.session_state.get('clusters') is not None:
    ...
    st.header("📈 Cumulative Summary by Cluster Type    summary_df = create_summary_df(clusters, depot_coord, circuity_factor) 
    st.header("📊 Cluster Summary")
    column_order = ['Cluster ID', 'Cluster Type', 'No. of Societies', 'Total Orders', 'Total Distance Fwd + Rev Leg (km)', 'Distance Between the Societies (km)', 'CPO (in Rs.)', 'Delivery Sequence']
    st.dataframe(summary_df.sort_values(by=['Cluster Type', 'Cluster ID'])[column_order])
    csv_buffer = BytesIO(); summary_df[column_order].to_csv(csv_buffer, index=False, encoding='utf-8')
